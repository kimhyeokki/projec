...
<%
	...
	if(val.equals("username")){
	...
	} else if(val.equals("userid")){
		sql ="select * from member  where userid like '%'||?||'%'";
		ps =  conn.conn.prepareStatement(sql);
		ps.setString(1,search);	
		...
		sql = "select * from member  where address like '%'||?||'%' or userid like '%'||?||'%' or username like '%'||?||'%' ";
		ps =  conn.conn.prepareStatement(sql);
		ps.setString(1,search);	
...
<table class="table">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><a href="../member/info.jsp?userID=<%= rs.getString("userID") %>"> <%= rs.getString("userID") %></a></td>
