...
--quiz1 이름 S로 끝나는 사원 데이터 출력
SELECT * FROM EMP WHERE ENAME LIKE '%S';
--quiz2 30번 부서에서 근무하는 사원중 job SALESMAN인 이름 사원번호 급여 부서번호 출력
SELECT EMPNO ,ENAME ,JOB ,SAL ,DEPTNO  FROM  EMP WHERE DEPTNO =30 AND JOB ='SALESMAN';
--quiz3 부서번호 20,30 에서 근무하고 있는 사원 중 급여가 2000 초과인 사원 (집합연산자 사용/사용하지 않고)
SELECT * FROM EMP WHERE DEPTNO =20 AND SAL >2000 UNION 
SELECT * FROM EMP WHERE DEPTNO =30 AND SAL >2000;

SELECT * FROM EMP WHERE DEPTNO =20 or DEPTNO=30 INTERSECT 
SELECT * FROM EMP WHERE SAL >2000;

SELECT * FROM EMP WHERE SAL >2000 AND DEPTNO =20 OR SAL >2000 AND DEPTNO =30;
...
SELECT * FROM EMP WHERE DEPTNO =20 AND SAL >2000 UNION 
SELECT * FROM EMP WHERE DEPTNO =30 AND SAL >2000;
--quiz4 NOT BETWEEN a and b 쓰지 않고 급여 2000이상, 3000이하 범위 이외 값을 가진 데이터 출력
SELECT *FROM  EMP WHERE SAL <2000 OR SAL >3000;

SELECT *FROM EMP WHERE not SAL>=2000 OR NOT SAL <=3000;
--quiz5 이름에 E가 포합 되어 있는 부서번호 30의 사원 중 급여가 1000~2000 사이가 아닌 이름 사원번호, 급여 부서번호 출려
...
--quiz6 추가 수당 존재하지 않고 상급자가 있고 직책이 MANAGER, CLERK인 사원 중 에서  이름이 두번째 글자 L이 아닌 사원의 정보 출력
SELECT  * FROM  EMP WHERE  COMM IS NULL AND MGR IS not NULL AND ENAME not LIKE '_L%' INTERSECT 
SELECT  * FROM  EMP WHERE JOB ='MANAGER' OR JOB ='CLERK';
SELECT  * FROM  EMP WHERE  COMM IS NULL AND MGR IS not NULL AND ENAME not LIKE '_L%' AND JOB ='MANAGER' OR JOB ='CLERK' ;
-- 강사님 정답내용
